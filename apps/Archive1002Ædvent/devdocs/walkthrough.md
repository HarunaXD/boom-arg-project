...
当前版本暂不在页面内展示前言。后续可单独制作开场页或外部文档，让玩家以「夜班归档巡查员 K」的身份进入调查。

角色设定：

- 玩家是临时接入镜像站的夜班归档巡查员。
- 调查对象是失踪粉丝「草莓奶昔」最后访问过的偶像应援官网。
- 表面任务是复原她的访问路径，确认她是否进入所谓的后台见面会。
- 实际风险是：玩家在复原路径时，也会被应援站作为新样本记录。

开场语气建议：

- 不直接说“这是器官采集陷阱”。
- 只提示玩家像普通粉丝一样浏览美图、榜单和周边防伪码。
- 让“不要登录、不要上传个人照片”作为调查守则出现，后续再被网页逐步反咬。
